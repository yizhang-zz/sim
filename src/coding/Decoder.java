...
class StateTrace {
    ...
    public StateTrace(State current, StateTrace parent, Symbol input, int time,
            boolean success, int seq) {
        this.state = current;
...
public class Decoder {
    ...
    private void insertNewState(LinkedList<StateTrace> list, State to,
        ...
        int len = list.size();
        if (len == 0) {
            StateTrace st = new StateTrace(to, parent, input, time, success,
                    seq);
            list.add(st);
        ...
        while (p <= q) {
            int mid = p + (q - p) / 2;
            if (list.get(mid).state.hashCode() < to.hashCode()) {
                p = mid + 1;
            } else if (list.get(mid).state.hashCode() > to.hashCode()) {
                q = mid - 1;
            } else {
    ...
    public ArrayList<DecodeResult> decode(boolean success, Symbol[] output,
        ...
        while (prune) {
            ...
            k--;
            st = history.get(k);
            System.out.println("back to time" + st.get(0).time);
