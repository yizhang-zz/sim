...
public class MVNModel implements Model {
	...
	private boolean allGood() {
		for (int i=0; i< m; i++)
			/* if a component is unkown, it must appear in symbolic constraint so it's lastType is required to be good */
	 ...
	 */
	public void makePrediction(int type, int[] ntype, double[] val, int[] status) {
		if (sentValues == null) {
			ts = 0;
			dim = m;
		...
		if (allKnown()) {
			...
			for (int j = 0; j < m; j++) {
				...
				if (ntype[j] == Interval.GOOD) {
					if (predictIndex[j] == -1) {
						/* type 0: x[i,j] = a */
						System.out.println(String.format("0:x[%d,%d] = %f", ts+1, j+1, sentValues
								.get(j, 0)));
					} else {
						/* type 1: a <= x[i,j] <= b*/
						System.out.println(String.format("1:%d,%d,%f,%f", ts+1, j+1, prediction
								.get(j - x, 0)
								- (epsilon1 + epsilon),  prediction.get(j - x, 0)
								+ (epsilon1 + epsilon)));
					}
				}
				else if (ntype[j] == Interval.BAD){
					if (predictIndex[j] == -1) {
						/* type 2: x[i,j] <a or x[i,j] >b */
						System.out.println(String.format("2:%d,%d,%f,%f",ts+1,j+1,sentValues
								.get(j, 0)-epsilon1,sentValues.get(j, 0)+epsilon1));
					}
					else {
						// loose bound
					}
			...
			Matrix C = predictMean
					.minusEquals(coef.times(mean.getMatrix(sentIndex, 0, 0))); // the constant term
			...
			for (int j = 0; j < m; j++) {
				...
				if (ntype[j] == Interval.GOOD) {
					double left = C.get(j-x, 0) - (epsilon+epsilon1);
					double right = C.get(j-x,0) + (epsilon+epsilon1);
